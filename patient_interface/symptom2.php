<?php
  ...
  $seldb = mysqli_select_db($db_link, "hospital")  
    or die("資料庫選擇失敗！");
  if (! (empty($_POST['ssid']))||empty($_POST['symptom2'])) {
  include('connectMySQL.php');
}
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $ssid = $_POST['ssid'];
    $symptom1=$_POST['symptom2'];
    $symptom_arr = array();
    $symptom_arr = $_POST['symptom2'];
    $symptom2 = implode(',', $symptom_arr);
  ...
  $symptom = mysqli_fetch_assoc($data);
    ...
    $sql_query = "UPDATE symptom SET symptom2=('$symptom2')  WHERE symptom_ssid='$ssid'";
